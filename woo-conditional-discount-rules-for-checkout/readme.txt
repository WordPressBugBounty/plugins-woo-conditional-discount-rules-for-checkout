=== Dynamic Pricing and Discount Rules for WooCommerce ===
Contributors: dots, niravcse006, jariwalasagar
Donate link: https://www.thedotstore.com/
Tags: category discount, dynamic pricing, woocommerce discount, dynamic discounts, bulk discount
Requires at least: 5.0
Tested up to: 6.8.1
WC tested up to: 9.8.4
Stable tag: 2.5.2
Requires PHP: 7.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

Conditional Discount Rules For WooCommerce Checkout Plugin will help you to create and manage complex discount rules based on your requirement.

== Description ==

Are your WooCommerce sales slowing down due to inflexible pricing and discount rules? Unlock your store's true sales potential with the <a href="https://bit.ly/39IhAEr" rel="nofollow ugc">Dynamic Pricing and Discount Rules for WooCommerce plugin</a>.

This plugin allows you to <strong>create tailor-made discounts to maximize your sales output</strong>. With an intuitive setup and a plethora of customizable rules, you get the power to attract and retain customers like never before.

[youtube https://www.youtube.com/watch?v=oeytloz7bVo]

<strong>Plugin Demo:</strong> <a href="https://pluginsdemo.thedotstore.com/woocommerce-conditional-discount-rules-for-checkout/" target="_blank">View Demo</a>

= 📒 KEY FEATURES =

<ul>
<li><strong>Bulk Discount:</strong> Set storewide, category-wise, or product-specific bulk discounts with ease.</li>
<li><strong>Quantity-Based Tiered Discounts:</strong> Offer enticing discounts based on the number of units purchased.</li>
<li><strong>Buy One Get One (BOGO) Offers:</strong> Set up various forms of BOGO offers, from "Buy X Get X" to conditional offers.</li>
<li><strong>Global Storewide Discount:</strong> Offer discounts on a global scale, perfect for end-of-season or festival sale.</li>
<li><strong>Location-Based Discount:</strong> Tailor discounts based on the customer's geographical location.</li>
<li><strong>Product-Specific Discount:</strong> Set unique discount rules for specific products.</li>
<li><strong>User Role-Based Discount:</strong> Offer special discounts based on user roles in your store.</li>
<li><strong>Cart-Based Discount:</strong> Apply discounts based on the total cart value.</li>
<li><strong>Payment Gateway-Based Discount:</strong> Encourage customers to use specific payment gateways with exclusive discounts.</li>
<li><strong>Weight-Based Discount:</strong> Offer discounts based on the product weight.</li>
</ul>

= 🚀 PLUGIN USE CASES =

<ol>
<li><strong>Seasonal Sales:</strong> Launch end-of-season sales with global discounts.</li>
<li><strong>Geographical Promotions:</strong> Run location-specific marketing campaigns with geo-targeted discounts.</li>
<li><strong>Customer Loyalty Program:</strong> Reward your loyal customers with user-role-based discounts.</li>
<li><strong>High Volume Purchases:</strong> Encourage bulk purchases with quantity-based tiered discounts.</li>
<li><strong>Payment Gateway Promotions:</strong> Partner with payment gateways to offer exclusive discounts.</li>
<li><strong>Weight-Based Shipping Promotion:</strong> Provide incentives for buying lightweight products with weight-based discounts.</li>
</ol>

= 📣 PREMIUM: Dynamic Pricing and Discount Rules — <a href="https://bit.ly/39IhAEr" target="_blank">Try the Pro Version Today!</a> =

= ⭐ REVIEWS (5-star by 100+ users) =

<ul>
<li>"Fantastic plugin! Makes setting rules and discounts a breeze."</li>
<li>"Great plugin for running different types of discounts for our WooCommerce store. Perfect role-based discounts!"</li>
<li>"The most complete plugin for dynamic discounts. Easy to understand and the support is incredibly fast."</li>
<li>"Amazing support. They replied within a few minutes and helped me solve my error right away!"</li>
<li>"A superb plugin with top-notch support. I am very pleased with their fast and adequate service."</li>
</ul>

= 👀 WHAT MAKES US STAND OUT =

We at Dotstore are committed to creating solutions that you can trust. 

Our <a href="https://www.thedotstore.com/plugins/" target="_blank">WooCommerce plugins</a> are built with precision and optimized for performance, delivering the functionality that is essential for successful e-commerce operations.

<ul>
<li><strong>Top-of-the-Line Customer Support:</strong> Our highly responsive and competent support team is always ready to assist you.</li>
<li><strong>Customization Flexibility:</strong> Our plugins are highly customizable and compatible with all major plugins.</li>
<li><strong>Simplicity:</strong> Our user-friendly plugins are designed with intuitive interfaces and easily accessible features.</li>
</ul>

With <a href="https://www.thedotstore.com/" target="_blank">Dotstore</a>, you're getting more than a plugin; you're partnering with a team dedicated to your e-commerce success.

= 📣 PREMIUM: Dynamic Pricing and Discount Rules — <a href="https://bit.ly/39IhAEr" target="_blank">Try the Pro Version Today!</a> =

= 🧩 SAMPLE EXAMPLE =

Bulk Discount — Create bulk discounts on entire store /categories /products. It can also be based on the order total.

<ul>
<li>Buy 10 quantity of any products in the store, get a 10% discount</li>
<li>Buy any 6 quantity from Category A, get a 10% discount</li>
<li>Buy 5 quantity of Product A and get a 10% discount</li>
</ul>

Product Specific discounts

<ul>
<li>Get a 20% discount on T-Shirts.</li>
<li>Get a 15% discount for any quantity of Shoe.</li>
<li>Buy 2 or more Tshirt – get 5$ discount each.</li>
</ul>

Category based discounts

<ul>
<li>Get a 20% discount on the shoe category.</li>
<li>Get a 25% discount on a men's shirt.</li>
</ul>

Cart subtotal based discounts

<ul>
<li>If Cart subtotal is $100 – 199 = 10 % discount.</li>
<li>If Cart subtotal is $200 – 299 = 20 % discount.</li>
<li>If Cart subtotal is $300 – 499 = 30% discount.</li>
</ul>

Country-based discounts

<ul>
<li>Give a 10% discount to US customers.</li>
<li>Give a 20% discount to Canadian customers.</li>
</ul>

User role-based discounts

<ul>
<li>20% discount to customers who belong to the user role “Subscriber”.</li>
<li>10% discount to customers who belong to the user role “wholesaler”.</li>
</ul>

Holiday Discount Offer

<ul>
<li>Halloween Day</li>
<li>Black Friday</li>
<li>Cyber Monday</li>
<li>Christmas, and Boxing day.</li>
</ul>

= 📣 PREMIUM: Dynamic Pricing and Discount Rules — <a href="https://bit.ly/39IhAEr" target="_blank">Try the Pro Version Today!</a> =

= 💁 How to’s Plugin Guide =

🔖 <a href="https://www.thedotstore.com/innovative-ways-in-which-enterprise-level-organisations-use-dynamic-pricing/" target="_blank">Innovative Ways in Which Enterprise-Level Organisations Use Dynamic Pricing</a>
🔖 <a href="https://www.thedotstore.com/choose-the-right-dynamic-pricing-plugin-for-woocommerce/" target="_blank">How to Choose the Right Dynamic Pricing Plugin for WooCommerce</a>
🔖 <a href="https://www.thedotstore.com/detailed-guide-woocommerce-dynamic-pricing-and-discount/" target="_blank">A Detailed Guide to The WooCommerce Dynamic Pricing and Discount Rules Plugin</a>
🔖 <a href="https://www.thedotstore.com/free-woocommerce-dynamic-pricing-discount-plugins/" target="_blank">20 Best Free WooCommerce Dynamic Pricing & Discount Plugins</a>
🔖 <a href="https://www.thedotstore.com/woocommerce-pricing-strategies/" target="_blank">Implementing Effective Pricing Strategies in WooCommerce</a>

= ☎️ CONTACT US =

<ul>
<li><strong>Free Plugin:</strong> Need Technical Help? <a href="https://wordpress.org/support/plugin/woo-conditional-discount-rules-for-checkout/" target="_blank">Click here</a></li>
<li><strong>Pro Plugin:</strong> PRE-SALE Query — <a href="https://www.thedotstore.com/support/" target="_blank">Click here</a> or <a href="https://docs.thedotstore.com/" target="_blank">Review Help Document</a></li>
</ul>

= 💎 EXPLORE OUR OTHER PLUGINS TO SUPERCHARGE YOUR WEBSITE =

👉 Premium WooCommerce Plugins list

<ol>
<li><a href="https://www.thedotstore.com/flat-rate-shipping-plugin-for-woocommerce" target="_blank" rel="noreferrer noopener">Flat-rate shipping plugins for WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-extra-fees-plugin" target="_blank" rel="noreferrer noopener">Extra Fees Plugin for WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/hide-shipping-method-for-woocommerce" target="_blank" rel="noreferrer noopener">Hide Shipping Method For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-product-attachment" target="_blank" rel="noreferrer noopener">Product Attachment For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-anti-fraud/" rel="noreferrer noopener" target="_blank">Blocker – Prevent Fake Orders And Blacklist Fraud Customers for WooCommerce</a></li>
<li><a href="http://www.thedotstore.com/woocommerce-enhanced-ecommerce-analytics-integration-with-conversion-tracking" rel="noreferrer noopener" target="_blank">Enhanced Ecommerce Google Analytics for WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-category-banner-management/" target="_blank" rel="noreferrer noopener">Category Banner Management for Woocommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-product-finder" target="_blank" rel="noreferrer noopener">Product Finder For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/mass-pages-posts-creator/" target="_blank" rel="noreferrer noopener">Mass Pages Posts Creator For WordPress</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-checkout-for-digital-goods/" target="_blank" rel="noreferrer noopener">WooCommerce Checkout For Digital Goods</a></li>
<li><a href="https://www.thedotstore.com/advance-menu-manager-wordpress/" target="_blank" rel="noreferrer noopener">Advance Menu Manager For WordPress</a></li>
<li><a href="https://www.thedotstore.com/page-visit-counter/" target="_blank" rel="noreferrer noopener">Page Visit Counter For WordPress</a></li>
</ol>

== Frequently Asked Questions ==

= Q. Can users receive more than one discount simultaneously? =

Ans: Absolutely! You can apply multiple discounts to a single order. This flexibility allows you to craft comprehensive discount strategies such as combining bulk discounts, tiered discounts, and BOGO offers as defined in your rule sets.

= Q. Can I control when a discount starts and ends? =

Ans: Yes, you have full control over the timing of your discounts. For each discount rule, you can specify both a start date and an end date, ensuring your promotions run exactly when you intend.

= Q. What types of discounts can I create with this plugin? =

Ans: With this powerful plugin, you have a variety of discount options at your disposal, including:

<ul>
<li><strong>Bulk Discounts:</strong> Apply discounts storewide or to specific categories, products, or attributes based on the quantity purchased.</li>
<li><strong>Tiered Discounts:</strong> Offer escalating discounts when customers purchase in bulk. For example, buying more units entitles them to greater discounts.</li>
<li><strong>BOGO Offers:</strong> Set up a variety of "Buy One, Get One" scenarios such as "Buy X Get X," "Buy X Get Y," or conditional offers like "Buy Product A and get 50% off Product B."</li>
</ul>

= Q. Can the plugin offer discounts based on the customer's location? =

Ans: Yes, the plugin enables you to set discounts based on the customer’s shipping location, perfect for tailoring your discounts to specific regions or markets.

= Q. Does the plugin support discounts based on the user's role? =

Ans: Indeed, you can configure discounts to apply only to certain user roles, providing an excellent way to offer exclusive perks to members, subscribers, or specific customer groups.

= Q. Can discounts be applied based on the payment method chosen? =

Ans: Yes, the plugin allows you to encourage the use of preferred payment methods by offering discounts when customers choose a specific gateway for their transaction.

= Q. Is it easy to manage and set up these discounts? =

Ans: Absolutely. One of the plugin’s strengths is its simplicity and user-friendly interface, which makes both setup and management of discount rules a breeze, ensuring an efficient process without the need for technical expertise.

= Q. What sets this plugin apart in terms of customer satisfaction? =

Ans: Based on customer reviews, what stands out are its comprehensive discount options, ease of use, and exceptional support. Customers have praised its flexibility, effectiveness in boosting sales, and responsive, helpful customer service, underscoring its reliability in delivering value to WooCommerce stores.

== Upgrade Notice ==

Automatic updates should work great for you.  As always, though, we recommend backing up your site prior to making any updates just to be sure nothing goes wrong.

== Screenshots ==

1. 
2. 
3. 
4. 
5. 
6. 
7. 

== Changelog ==
= 2.5.2 - 08-05-2025 =
* [New] – Compatible with CURCY – Multi-Currency for WooCommerce plugin
* [Bug Fix] Minor fixes related to the Advanced Discount Price Rules
* [Enhancement] – Compatible with WooCommerce 9.8.x
* [Enhancement] – Compatible with WordPress 6.8.x

= 2.5.1 - 10-03-2025 =
* [Fixed] - Resolved minor bugs to enhance stability  
* [New] - Introduced variable product support in conditional rules  
* [Enhancement] - Updated Freemius SDK to v2.11.0  
* [Enhancement] - Ensured compatibility with WooCommerce 9.6.x  
* [Enhancement] - Verified compatibility with WordPress 6.7.x  

= 2.5.0 =
* [Enhancement] Plugin UI Enhancement
* [Enhancement] Compatible with WooCommerce 9.1.x
* [Enhancement] Compatible with WordPress 6.6.x

= 2.4.2 =
* Fixed - Minor bug fixed
* [Enhancement] Plugin UI Enhancement
* [Enhancement] Compatible with WooCommerce 8.8.x
* [Enhancement] Compatible with WordPress 6.5.x

= 2.4.1 =
* [Bug Fix] Minor fixes related to the Product specific condition
* [Bug Fix] Minor fixes related to the plugin design
* [Enhancement] Quick fix related to the Cross Site Request Forgery (CSRF) vulnerability
* [Enhancement] Compatible with WooCommerce 8.1.x
* [Enhancement] Compatible with WordPress 6.3.x

= 2.4.0 =
* [Fixed] Discount message raise in_array() condition issue
* [Fixed] The product table plugin shop page table is not working when the discount plugin is active. 
* [Enhancement] New design implementation
* [Enhancement] Add TinyMCE editor for discount content
* [Enhancement] Compatible with WooCommerce HPOS/COT
* [Enhancement] Compatible with WooCommerce Product Table (Barn2 Plugins)
* [Enhancement] Compatible with WooCommerce 8.0.x
* [Enhancement] Compatible with WordPress 6.3.x

= 2.3.3.1 =
* Enhancement - Quick fix related to the Cross Site Scripting (XSS) vulnerability
* Enhancement - Compatible with WooCommerce 7.9.x
* Enhancement - Compatible with WordPress 6.2.x

= 2.3.3 =
* New - Enable/disable toggle on listing
* New - Text color and background color for the discount message
* Fixed - Minor bug fixed
* Compatible with WooCommerce 7.3.x
* Compatible with WordPress 6.1.x

= 2.3.2 =
* Fixed - Minor bug fixed
* Fixed - Licensing/Trademark Violation issue has been fixed.
* Compatible with WooCommerce 7.1.x
* Compatible with WordPress 6.1.x

= 2.3.1 =
* Fixed - Minor bug fixes related to the Currency format
* Fixed - Improved discount rules loading time
* Update - Compatible with WPML plugin
* Update – UI design fixes
* Update – Compatible with WooCommerce 6.8.x
* Update – Compatible with WordPress 6.0.x

= 2.3.0 =
* Add - Apply discount on line item count
* Update – Compatible with WooCommerce 6.3.x
* Update – Compatible with WordPress 5.9.x
* Enhancement – Minor bug fixes
* Enhancement – Update UI chnages

= 2.2.3 =
* Enhancement – Security patch
* Update – Compatible with WooCommerce 6.3.x
* Update – Compatible with WordPress 5.9.x

= 2.2.2 =
* New - Discount message only for specific products
* Fixed - User role based rule not working
* Compatible with WooCommerce 6.0.x
* Compatible with WordPress 5.8.x

= 2.2.1 =
* Compatible with WooCommerce 6.0.x
* Compatible with WordPress 5.8.x

= 2.2.0 - 20-10-2021 =
* New - Include or exclude sale products from discount rule
* New - Display custom message on product details page 
* New - Search discount name from list in setting page
* Fixed - Minor bug fixed
* Compatible with WooCommerce 5.8.x
* Compatible with WordPress 5.8.x


= 2.1.0 – 19-08-2021 =
* New - Add time specific discount
* Fixed - Minor bug fixed
* Compatible with WooCommerce 5.6.x
* Compatible with WordPress 5.8.x

= 2.0.9 – 27-07-2021 =
* Compatible with WooCommerce 5.5.x
* Compatible with WordPress 5.8.x

= 2.0.8 – 02-07-2021 =
* Compatible with WooCommerce 5.4.x
* Compatible with WordPress 5.7.x

= 2.0.7 – 04-05-2021 =
* Compatible with WooCommerce 5.2.x
* Compatible with WordPress 5.7.x

= 2.0.6 – 02-02-2021 =
* Compatible with WooCommerce 4.9.x
* Compatible with WordPress 5.6.x

= 2.0.5 – 17-12-2020 =
* Compatible with WooCommerce 4.8.x
* Compatible with WordPress 5.6.x
* Minor bug fixed

= 2.0.4 - 25-11-2020 =
* Minor bug fixed

= 2.0.3 - 20-11-2020 =
* Maintenance Release

= 2.0.2 - 29-09-2020 =
* Compatible with WooCommerce 4.5.x
* Compatible with WordPress 5.5.x

= 2.0.1 - 08-05-2020 =
* Compatible with WooCommerce 4.1.x
* Compatible with WordPress 5.4.x

= 2.0.0 - 29-11-2019 =
* Minor big fixing.
* Freemius SDK integration
* VIP compatible

= 1.0.2 - 19-11-2019 =
* Compatible with Wordpress 5.3.x
* Compatible with WooCommerce 3.8.x

= 1.0.1 - 20-09-2019 =
* Maintenance Releases

= 1.0 - 10-08-2019 =
* Initial Release.